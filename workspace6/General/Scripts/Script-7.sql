...
SELECT EMP.ENAME , EMP.SAL , DEPT.DNAME , SALAGRADE.GRADE
FROM EMP.ENAME 
JOIN DEPT.DNAME ON EMP.deptno = DEPT.deptno
JOIN salgrade ON EMP.SAL BETWEEN SALAGRADE.LOSAL AND SALAGRADE.HISAL
...
AND LOC = 'CHICAGO'
AND SAL > (SELECT AVG(SAL) FROM EMP, DEPT WHERE EMP.DEPTNO = DEPT.DEPTNO LOC = 'CHICAGO')
ORDER BY SAL DESC;
...
SELECT DEPT.DNAME AS 부서명, EMP.ENAME AS 사원명, EMP.JOB AS 업무
FROM EMP, DEPT
WHERE EMP.DEPTNO = DEPT.DEPTNO
AND EMP.JOB = 'SALESMAN'
AND EMP.DEPTNO IN (SELECT DEPTNO FROM EMP WHERE JOB = 'SALESMAN' GROUP BY DEPTNO  HAVING COUNT(*) >= 2)
...
AND EMP.SAL = (SELECT MAX(SAL) FROM EMP WHERE COMM IS NULL)
AND EMP.SAL BETWEEN SALGRADE.LOWER_BOUND AND SALGRADE.UPPER_BOUND;


-- ■ 12. SMITH의 관리자(MGR)의 이름과 부서명, 근무지역을 출력하시오. ■


SELECT MANAGER.ENAME AS 관리자명, DEPT.DNAME AS 부서명, DEPT.LOC AS 근무지역
FROM EMP SMITH, EMP MANAGER, DEPT
WHERE SMITH.ENAME = 'SMITH'
AND MANAGER.EMPNO = SMITH.MGR
...
SELECT EMPLOYEE_ID, LAST_NAME,JOB_ID,SALARY
FROM EMPLOYEES
WHERE SALARY > (SELECT SALARY FROM EMP WHERE LAST_NAME = 'Kochhar');
...
SELECT EMPLOYEE_ID,LAST_NAME,JOB_ID,DEPARTMENT_ID
FROM EMPLOYEES
WHERE SALARY = SELECT MIN(SALARY) FROM EMPLOYEES WHERE  JOB_ID
ORDER BY e.JOB_ID;
...
SELECT EMPLOYEES.FIRST_NAME, EMPLOYEES.JOB_ID, DEPARTMENTS.DEPARTMENT_NAME, LOCATIONS.CITY
FROM EMPLOYEES
...
SELECT EMPLOYEES.EMPLOYEE_ID, EMPLOYEES.FIRST_NAME, EMPLOYEES.JOB_ID, DEPARTMENTS.DEPARTMENT_NAME
FROM EMPLOYEES
JOIN DEPARTMENTS ON EMPLOYEES.DEPARTMENT_ID = DEPARTMENTS.DEPARTMENT_ID
WHERE EMPLOYEES.SALARY = (SELECT MIN(SALARY) FROM EMPLOYEES WHERE JOB_ID = EMPLOYEES.JOB_ID)
