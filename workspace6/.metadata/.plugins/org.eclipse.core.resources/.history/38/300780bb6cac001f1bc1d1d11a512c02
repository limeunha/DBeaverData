...
JOIN CLASS T2 ON (T1.CLASS_NO = T2.CLASS_NO)
JOIN STUDENT T3 ON (T1.STUDENT_NO = T3.STUDENT_NO);

--집합 연산자

--중복데이터를 포함해서 전체 테이블의 합칩합 결과를 가져옴
SELECT * FROM ACADEMY_A
UNION ALL
SELECT * FROM ACADEMY_B;


--중복데이터를 포함하지 않고 전체 테이블의 합칩합 결과를 가져옴
SELECT * FROM ACADEMY_A
UNION
SELECT * FROM ACADEMY_B;

--중복데이터를 제거해서 교집합 데이터만 가져온다
SELECT * FROM ACADEMY_A
INTERSECT
SELECT * FROM ACADEMY_B;

--왼쪽 테이블에서 교집합된 데이터를 제거하고 가져온다.
SELECT * FROM ACADEMY_A
MINUS
SELECT * FROM ACADEMY_B;
