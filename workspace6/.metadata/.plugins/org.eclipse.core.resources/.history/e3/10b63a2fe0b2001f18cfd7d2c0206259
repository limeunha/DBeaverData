...
-- ■ 3. EMPLOYEES 테이블에서 부서번호로 정렬한 후 부서번호가 같을 경우 급여가 많은 순으로 정렬하여 사원번호, 성명, 업무J, 부서번호, 급여를 출력하여라. ■
SELECT * FROM EMPLOYEES;
...
FROM EMPLOYEES
WHERE DEPARTMENT_ID, JOB_ID, SALARY DESC;
...
SELECT DEPARTMENT_ID,DEPARTMENT_NAME, DEPARTMENT_NAME || ' ' || LOCATION_ID AS DEPARTMENT_LOCATION
FROM DEPARTMENTS;
...
SELECT EMPLOYEE_ID,LAST_NAME,JOB_ID,SALARY,DEPARTMENT_ID
FROM EMPLOYEES
WHERE UPPER(SUBSTR(LAST_NAME, 1, 1)) > 'K' 
AND UPPER(SUBSTR(LAST_NAME, 1, 1)) < 'Y'
ORDER BY LAST_NAME;


-- ■ 9. EMPLOYEES 테이블에서 20번 부서 중 이름의 길이 및 급여의 자릿수를 사원번호, 이름, 이름의 자릿수, 급여 급여의 자릿수를 출력하라. ■

SELECT EMPLOYEE_ID, LAST_NAME,LENGTH(LAST_NAME) AS NAME_LENGHT, SALARY,LENGTH(TO_CHAR(SALARY)) AS SALARY_LENGTH
FROM  EMPLOYEES
WHERE DEPARTMENT_ID = 20;


-- ■ 10. EMPLOYEES 테이블에서 이름 중 'e' 자의 위치를 출력하라. ■
SELECT LAST_NAME, INSTR(LAST_NAME = 'e') E_POSITION
FROM EMPLOYEES
...
SELECT EMPLOYEE_ID, LAST_NAME MOD(SALARY, 30) SALARY_MOD
FROM EMPLOYEES
WHERE DEPARTMENT_ID = 80;

-- ■ 13. EMPLOYEES 테이블에서 30번 부서 중 이름과 담당 업무를 연결하여 출력하여라. 단 담당 업무를 한 줄 아래로 출력하라. ■
SELECT LAST_NAME || CHR(10) || JOB_ID AS NAME_JOB
FROM EMPLOYEES
WHERE DEPARTMENT_ID = 30;


-- ■  14. EMPLOYEES 테이블에서 현재까지 근무일 수가 몇월 몇일 인가를 출력하여라. 단 근무 일수가 많은 사람 순으로 출력하여라. ■
SELECT HIRE_DATE, LAST_NAME, EMPLOYEE_ID,TRUNC(SYSDATE - HIRE_DATE) AS WORK_DAYS
FORM EMPLOYEES
ORDER BY WORK_DAYS DESC;

-- ■ 15. EMPLOYEES 테이블에서 부서 50에서 급여 앞에 $를 삽입하고 3자리마다 , 를 출력하라. ■
SELECT EMPLOYEE_ID, LAST_NAME, TO_CHAR(SALARY, '$999,999,999') AS FORMATTED_SALARY
FROM EMPLOYEES
WHERE DEPARTMENT_ID = 50;


-- ■ 16. EMPLOYEES 테이블에서 모든 SALESMAN(SA_MAN)에 대하여 급여의 평균,최고액,최저액,합계를 구하여 출력하여라. ■
SELECT AVG(SALARY) AS AVG_SALARY,MAX(SALARY) AS MAX_SALARY,MIN(SALARY) AS MIN_SALARY,SUM(SALARY) AS TOTAL_SALARY
FROM EMPLOYEES
WHERE JOB_ID = 'SA_MAN';


-- ■ 17. EMPLOYEES 테이블에 등록되어 있는 인원수, 보너스가 NULL이 아닌 인원수, 보너스의 평균, 등록되어 있는 부서의 수를 구하여 출력하라. ■
SELECT COUNT(*) AS TOTAL_EMPLOYEES, COUNT(BONUS) AS NON_NULL_BONUS_COUNT,AVG(BONUS) AS AVG_BONUS,COUNT(DISTINCT DEPARTMENT_ID) AS TOTAL_DEPARTMENTS 
FROM EMPLOYEES;


-- ■ 18. EMPLOYEES 테이블에서 부서별로 인원수, 평균 급여, 최저급여, 최고급여, 급여의 합을 구하여 출력하라. ■
SELECT DEPARTMENT_ID,COUNT(*) AS EMPLOYEE_COUNT, AVG(SALARY) AS AVG_SALARY, MIN(SALARY) AS MIN_SALARY,MAX(SALARY) AS MAX_SALARY,SUM(SALARY) AS TOTAL_SALARY
FROM EMPLOYEES
GROUP BY DEPARTMENT_ID; 
...
SELECT DEPARTMENT_ID,JOB_ID,AVG(SALARY),SUM(SALARY),COUNT(*) AS EMPLOYEE_COUNT
FROM EMPLOYEES
GROUP BY DEPARTMENT_ID, JOB_ID ;


-- ■ 21. EMPLOYEES 테이블에서 부서 인원이 4명보다 많은 부서의 부서번호, 인원수, 급여의 합을 구하여 출력하여라(GROUP BY, HAVING) ■
SELECT DEPARTMENT_ID ,SUM(SALARY),COUNT(*) AS EMPLOYEE_COUNT
FROM EMPLOYEES
GROUP BY DEPARTMENT_ID
HAVING COUNT(*) > 4;
...
SELECT ENAME, LPAD(SALARY, 15, '$') AS SALARY
FROM EMP;
...
WHERE EMPLOYEES.FIRST_NAME = 'Steven' AND EMPLOYEES.LAST_NAME = 'King';
...
SELECT EMPLOYEES.EMPLOYEE_ID, EMPLOYEES.LAST_NAME, EMPLOYEES.SALARY, DEPARTMENTS.DEPARTMENT_NAME, EMPLOYEES.JOB_ID
FROM EMPLOYEES
JOIN DEPARTMENTS
ON EMPLOYEES.DEPARTMENT_ID = DEPARTMENTS.DEPARTMENT_ID
...
SELECT EMPLOYEES.EMPLOYEE_ID, EMPLOYEES.LAST_NAME, EMPLOYEES.JOB_ID, EMPLOYEES.DEPARTMENT_ID, DEPARTMENTS.DEPARTMENT_ID, DEPARTMENTS.DEPARTMENT_NAME, DEPARTMENTS.LOCATION_ID
FROM EMPLOYEES
...
SELECT EMPLOYEES.EMPLOYEE_ID, EMPLOYEES.LAST_NAME, EMPLOYEES.JOB_ID, EMPLOYEES.SALARY, EMPLOYEES.MANAGER_ID
FROM EMPLOYEES
...
SELECT EMPLOYEES.EMPLOYEE_ID, EMPLOYEES.LAST_NAME, EMPLOYEES.JOB_ID, EMPLOYEES.SALARY, EMPLOYEES.MANAGER_ID
FROM EMPLOYEES
RIGHT JOIN EMPLOYEES AS MANAGER
ON EMPLOYEES.MANAGER_ID = MANAGER.EMPLOYEE_ID
WHERE MANAGER.EMPLOYEE_ID = 108
ORDER BY EMPLOYEES.SALARY DESC;
